 ...
 *
 * Hiring twin of Leads → Tours. Same operational list shape (upcoming /
 * past / cancelled / all + search), retargeted at interview slots for
 * prospective staff instead of tour slots for prospective families.
 * Shares `useCandidatesData` so counts stay aligned with the Candidates
 * Dashboard and Procedures pages.
 */
...
export default function CandidatesInterviewsPage() {
  const { mobileMode } = useViewMode()
  const { interviews, loading } = useCandidatesData()
    ...
    <div>
      ...
      <div className="mb-6 flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Interviews</h2>
            <p className="text-gray-500 mt-0.5 text-sm">
              Scheduled meetings, phone screens, and follow-ups for potential new staff.
            </p>
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        {loading ? (
          <div className="text-center py-10 text-gray-400 text-sm">Loading interviews...</div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-14 px-6">
            <div className="w-12 h-12 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-3">
              <Calendar className="w-5 h-5 text-gray-300" />
            </div>
            <p className="text-sm font-medium text-gray-700">No interviews match this view</p>
            <p className="text-xs text-gray-400 mt-1 max-w-xs mx-auto">
              Interviews booked via the applicant intake flow will appear here grouped by status.
            </p>
          </div>
        ) : (
              ...
              return (
                <div key={t.id || `${name}-${t.scheduled_at}`} className="flex items-center gap-4 px-4 py-3 hover:bg-gray-50 transition-colors">
                  <div className="w-9 h-9 rounded-lg bg-indigo-50 flex items-center justify-center shrink-0">
                    <UserSearch className="w-4 h-4 text-indigo-600" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-medium text-gray-800 truncate">{name}</p>
                      {t.role && (
                        <span className="inline-block px-2 py-0.5 rounded-full text-[11px] font-medium bg-indigo-50 text-indigo-600 shrink-0">
                          {t.role}
                        </span>
                      )}
      ...
      <div className="mt-6 bg-indigo-50/60 border border-indigo-100 border-l-4 border-l-indigo-500 rounded-xl p-4 flex gap-3">
        ...
        <div className="text-xs text-gray-600">
          ...
          <p>
            Interviews read from the <code className="bg-white px-1 rounded text-[11px]">interviews</code> table and
            share the same counts shown on the Candidates Dashboard.
          </p>
        </div>
      </div>
    </div>
